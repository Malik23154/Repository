...
#program wywołujący okno tak to z góry też wywołuje
...
coverFrame = Frame (root,bd = 20, pady=20, relief = RIDGE)
coverFrame.grid()
...
class Calculator():
    ...
    def cos(self):
        self.result = False
        self.current = (math.cos(float(entDisplay.get())))
        self.display(self.current)
...
btnSum=Button(MainFrame, width = 6, height=2, font=('arial', 16, 'bold' ), bd=4, text = "-", 
bg='cadet blue', 
                command = lambda:added_value.operation("sub"))
btnSum.grid(row = 4, column= 3, pady = 1)
